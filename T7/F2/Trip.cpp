...
Trip::Trip(int _id, Passenger* _passenger, Place _origin, Place _destination, double _cost){
    id = _id;
    origin = _origin;
...
string Trip::to_string(){
    ...
    os << id << SPACE << passenger->get_username() << SPACE << origin.name << SPACE << destination.name
       << SPACE << fixed << cost << SPACE << status;
    return os.str();
}
