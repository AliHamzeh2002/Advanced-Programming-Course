...
string TaxiManager::handle_get_trips(Command command){
    ...
    try{
        ...
        bool sort_by_cost = (command.get_parameter(SORT_BY_COST) == YES) ? true : false;
        return get_all_trips(sort_by_cost);
    }
    catch(...){
        throw;
    }
...
string TaxiManager::get_all_trips(bool sort_by_cost){
    ostringstream os;
    vector<Trip*> trips = data_manager.get_trips();
    if (sort_by_cost)
        sort(trips.begin(), trips.end(), trips_compare);
...
bool TaxiManager::trips_compare(Trip* trip1, Trip* trip2){
...
string TaxiManager::handle_get_command(Command command){
    ...
    if (command_name == COST_COMMAND)
        return handle_cost(command);
    throw NotFoundError();
}

double TaxiManager::calc_distance(Location origin, Location destination){
    return DIST_COEFFICIENT * sqrt(pow(origin.latitude - destination.latitude,2) +
                                    pow(origin.longitude - destination.longitude,2));
}

double TaxiManager::calc_cost(Location origin, Location destination, bool in_hurry){
    double distance = calc_distance(origin, destination);
    double price_coefficient = PRICE_COEFFICIENT;
    if (in_hurry)
        price_coefficient *= 1.2;
    return distance * (origin.traffic_coefficient + destination.traffic_coefficient) * price_coefficient;
}

string TaxiManager::handle_cost(Command command){
    string username = command.get_parameter(USERNAME);
...
string TaxiManager::handle_post_trip(Command command){
    ...
    Location origin = data_manager.find_location(origin_str);
    double cost = calc_cost(origin, dest, in_hurry);
    data_manager.add_new_trip(new Trip(available_id, passenger, {origin_str,origin}, {dest_str, dest}, cost));
